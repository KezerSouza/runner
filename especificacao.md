...
- [x] O CLI deve aceitar comandos para criação e validação de assinatura
- [x] O CLI deve invocar o assinador.jar com os parâmetros fornecidos
- [x] O CLI deve permitir a invocação direta do assinador.jar (modo local/CLI)
- [x] O CLI deve permitir a invocação do assinador.jar via HTTP (modo servidor)
- [x] O CLI deve exibir o resultado da operação de forma legível
- [x] O CLI deve iniciar o assinador.jar no modo servidor usando a porta padrão quando não orientado de forma diferente
- [x] O CLI deve detectar se instância do assinador.jar já está em execução no modo servidor e usar essa instância, se não orientado de forma diferente. 
- [x] O CLI deve fazer uso do assinador.jar no modo servidor quando não orientado para usar o modo local
- [x] O CLI deve interromper a execução do assinador.jar na porta padrão ou outra indicada.
- [x] O CLI deve permitir a requisição de interrupção programada do assinador.jar após o número de minutos fornecidos sem interação
...
**Critérios de aceitação:**
- [x] O assinador.jar deve validar todos os parâmetros conforme especificações (veja seção de referências abaixo)
- [x] O assinador.jar deve simular a criação de assinatura retornando resposta pré-construída quando parâmetros válidos
- [x] O assinador.jar deve simular validação de assinatura retornando resultado pré-determinado
- [x] O assinador.jar deve suportar interação com dispositivo criptográfico (token/smart card) via interface PKCS#11
- [x] O assinador.jar deve retornar mensagens de erro claras quando parâmetros forem inválidos
...
**Critérios de aceitação:**
- [x] O CLI deve verificar se a porta padrão, empregada pelo Simulador, 8443, está disponível antes de tentar iniciá-lo.
- [x] O CLI deve permitir iniciar o Simulador.
- [x] O CLI deve permitir parar o Simulador (endpoint /shutdown).
- [x] O CLI deve exibir o status atual do Simulador (ou que não está em execução). O status pode ser obtido via endpoint /api/info.
- [x] O Simulador (simulador.jar) deve ser obtido dinamicamente pelo CLI, baixando a versão mais recente disponível no repositório da disciplina (GitHub Releases).
- [x] O CLI deve baixar o JRE caso não esteja disponível no diretório .hubsaude a partir do Eclipse Temurin (Adoptium).
- [x] O CLI não deve baixar o Simulador (simulador.jar) se a versão mais recente já estiver disponível localmente.
...
**Critérios de aceitação:**
- [x] O sistema deve detectar se o JDK está presente na máquina (na versão exigida)
- [x] O sistema deve baixar o JDK compatível quando ausente
- [x] O sistema deve disponibilizar o JDK baixado para uso próprio ou seja, pelo Assinador e Simulador
- [x] O download deve funcionar nas três plataformas
...
**Critérios de aceitação:**
- [x] Disponibilizar binário para Windows (amd64)
- [x] Disponibilizar binário para Linux (amd64)
- [x] Disponibilizar binário para macOS (amd64)
- [x] Distribuir via GitHub Releases
- [x] Incluir checksums SHA256 para verificação de integridade
- [x] Utilizar versionamento semântico (SemVer)
...
6. **Artefatos executáveis**
   - Binários pré-compilados para as três plataformas suportadas:
     ...
     - `assinatura-<versão>-darwin-amd64` (macOS)
     - `simulador-<versão>-windows-amd64.exe` (Windows)
     - `simulador-<versão>-linux-amd64` (Linux)
     - `simulador-<versão>-darwin-amd64` (macOS)
   - Distribuídos via **GitHub Releases**
   - Cada release contém assinatura dos artefatos publicados com Cosign.
...
### 8.3. Mapeamento para a especificação FHIR

A seção 10 referencia as especificações FHIR da SES-GO para criar e validar assinaturas digitais reais. No contexto desta simulação, os parâmetros aceitos pelo `assinador.jar` mapeiam para os conceitos FHIR da seguinte forma:

| Parâmetro simulado | Conceito FHIR equivalente | Observação |
|--------------------|--------------------------|------------|
| `--content` | Bundle/Provenance FHIR JSON | Na simulação aceita qualquer texto não-vazio; no sistema real seria um recurso FHIR |
| `--token` | Identificador do slot PKCS#11 | Opcional; mapeia ao `Identifier` do dispositivo criptográfico |
| `--signature` | JWS JSON Serialization (RFC 7515) | Na simulação é uma string fixa; no sistema real seria um JWS completo |

A implementação real dos campos completos (cadeia de certificados X.509, timestamp de referência, política de assinatura, âncoras de confiança) está fora do escopo deste trabalho, conforme definido na seção 4.2.
...
```
assinatura-0.0.6-linux-amd64
assinatura-0.0.6-linux-amd64.sha256
assinatura-0.0.6-linux-amd64.sig
assinatura-0.0.6-linux-amd64.pem
```
...
```bash
cosign verify-blob \
  --certificate assinatura-0.0.6-linux-amd64.pem \
  --signature assinatura-0.0.6-linux-amd64.sig \
  assinatura-0.0.6-linux-amd64
```
